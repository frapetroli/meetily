...
impl RecordingManager {
    ...
    /// `denoiser` is `Some` only when `transcript_settings.denoising_enabled` is on
    /// (ADR-0027) and the caller already constructed it via `denoising::prepare_if_enabled`
    /// -- `None` means zero denoising overhead, independent of `diarization_sender`.
    /// `denoise_debug_paths`, when `Some` (only meaningful alongside `denoiser: Some`),
    /// are temp-file destinations for a debug copy of the denoised ASR/diarization
    /// signal, generated by the caller since the real meeting folder isn't created
    /// yet at this point (see `denoising::DebugWavWriter`).
    pub(crate) async fn start_recording(
        ...
        if let Err(error) = self.pipeline_manager.start(
            ...
            denoiser, // Transforms the tap in place before it forks, only if denoising is enabled
            denoise_debug_paths, // Temp destinations for the denoised debug wav files, if any
            mic_name,
            mic_kind,
            sys_name,
            sys_kind,
